...
function loadState(){S=Object.assign({k:{},w:{},xp:0,days:{},sound:null,voice:null},(()=>{try{return JSON.parse(localStorage.getItem(KEY()))||JSON.parse(localStorage.getItem('kana.duo.v2'))||{}}catch{return{}}})())}
...
function persist(){try{localStorage.setItem(SKEY(),JSON.stringify({steps,si,score,combo,newK,ts:Date.now()}))}catch{}}
function clearPersist(){try{localStorage.removeItem(SKEY())}catch{}}
...
function finish(){
  ...
  S.xp+=xp;S.days[today()]=(S.days[today()]||0)+1;save();clearPersist();
